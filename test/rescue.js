...
describe('Rescue test', function() {
  ...
  before(async () => {
    ({ token_contract, blacklister, rescuer, firelight_vault, users, utils, config } = await loadFixture(
  ...
  it('reverts if the caller has not RESCUER_ROLE granted', async () => {
    const rescueAttempt = firelight_vault.connect(blacklister).rescueSharesFromBlacklisted(users[0].address, users[2].address)
    await expect(rescueAttempt).to.be.revertedWithCustomError(firelight_vault, 'AccessControlUnauthorizedAccount')
  })

  it('reverts if rescuer tries to rescue shares from a NOT blacklisted user', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueSharesFromBlacklisted(users[0].address, users[2].address)
    await expect(rescueAttempt).to.be.revertedWithCustomError(firelight_vault, 'NotBlacklistedAddress')
  })

  it('blacklister successfully adds users to the blacklist', async () => {
    await firelight_vault.connect(blacklister).addToBlacklist(users[0].address)
    expect(await firelight_vault.isBlacklisted(users[0].address)).to.eq(true)

    await firelight_vault.connect(blacklister).addToBlacklist(users[1].address)
    expect(await firelight_vault.isBlacklisted(users[1].address)).to.eq(true)
  })

  it('reverts if rescuer tries to rescue shares to a blacklisted user', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueSharesFromBlacklisted(users[0].address, users[0].address)
    await expect(rescueAttempt).to.be.revertedWithCustomError(firelight_vault, 'BlacklistedAddress')
  })

  it('reverts if rescuer tries to rescue shares to a zero address', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueSharesFromBlacklisted(users[0].address, ethers.ZeroAddress)
    await expect(rescueAttempt).to.be.revertedWithCustomError(firelight_vault, 'ERC20InvalidReceiver')
  })

  it('reverts if rescuer tries to rescue shares from blacklisted user with no shares', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueSharesFromBlacklisted(users[1].address, users[2].address)
    await expect(rescueAttempt).to.be.revertedWithCustomError(firelight_vault, 'InsufficientShares')
  })

  it('rescuer transfers shares from blacklisted user', async () => {
    const prevTs = await time.latest()
    expect(await firelight_vault.balanceOf(users[0].address)).to.eq(DEPOSIT_AMOUNT)

    const rescueTrx = firelight_vault.connect(rescuer).rescueSharesFromBlacklisted(users[0].address, users[2].address)
    await expect(rescueTrx).to.emit(firelight_vault, 'SharesRescuedFromBlacklisted').withArgs(
  ...
  it('reverts if rescuer tries to rescue shares from blacklisted user again', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueSharesFromBlacklisted(users[0].address, users[2].address)
    await expect(rescueAttempt).to.be.revertedWithCustomError(firelight_vault, 'InsufficientShares')
  })
  ...
  it('reverts if the caller has not RESCUER_ROLE granted', async () => {
    const rescueAttempt = firelight_vault.connect(blacklister).rescueWithdrawFromBlacklisted(users[2].address, rescuer.address, [1,2])
    await expect(rescueAttempt).to.be.revertedWithCustomError(firelight_vault, 'AccessControlUnauthorizedAccount')
  })

  it('reverts if rescuer tries to rescue pending withdrawals from a blacklisted user to another blacklisted user', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueWithdrawFromBlacklisted(users[1].address, users[1].address, [1,2])
    await expect(rescueAttempt).to.revertedWithCustomError(firelight_vault,'BlacklistedAddress')
  })

  it('reverts if rescuer tries to rescue pending withdrawals from a blacklisted user to zero address', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueWithdrawFromBlacklisted(users[1].address, ethers.ZeroAddress, [1,2])
  ...
  it('reverts if rescuer tries to rescue pending withdrawals from a NOT blacklisted user', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueWithdrawFromBlacklisted(users[2].address, rescuer.address, [1,2])
    await expect(rescueAttempt).to.revertedWithCustomError(firelight_vault,'NotBlacklistedAddress')
  })

  it('blacklister successfully adds a user with pending a withdraw to the blacklist', async () => {
    await firelight_vault.connect(blacklister).addToBlacklist(users[2].address)
    expect(await firelight_vault.isBlacklisted(users[2].address)).to.eq(true)
  })

  it('blacklisted user cannot claim withdraw anymore', async () => {
  ...
  it('reverts if rescuer tries to rescue pending withdrawals if one or more periods have no pending amount', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueWithdrawFromBlacklisted(users[2].address, rescuer.address, [2,3,4])
  ...
  it('reverts if rescuer tries to rescue pending withdrawals if one or more periods were already claimed', async () => {
    const rescueAttempt = firelight_vault.connect(rescuer).rescueWithdrawFromBlacklisted(users[2].address, rescuer.address, [2,1])
    await expect(rescueAttempt).to.revertedWithCustomError(firelight_vault,'AlreadyClaimedPeriod').withArgs(1)
  })

  it('rescuer successfully rescues the two pending withdrawals from blacklisted user to itself', async () => {
    const periods = [2,3]
    const rescuedShares = [PART_WITHDRAW_AMOUNT, PART_WITHDRAW_AMOUNT]

    const rescueTrx = firelight_vault.connect(rescuer).rescueWithdrawFromBlacklisted(users[2].address, rescuer.address, periods)
    await expect(rescueTrx).to.emit(firelight_vault, 'WithdrawRescuedFromBlacklisted').withArgs(
      users[2].address, rescuer.address, periods, rescuedShares
    )
  })

  it('move forward one more period and rescuer successfully claims all rescued shares', async () => {
    ...
    // Move one period to make second withdraw available
    await time.increase(config.period_duration)
